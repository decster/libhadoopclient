...
uint32_t crc32_sb8(uint32_t, const uint8_t *, size_t);
uint32_t crc32c_sb8(uint32_t, const uint8_t *, size_t);

uint32_t Checksum::init(ChecksumType type) {
  switch (type) {
  case CHECKSUM_NONE:
    return 0;
  case CHECKSUM_CRC32:
...
void Checksum::update(ChecksumType type, uint32_t & value, const void * buff, uint64_t length) {
  switch (type) {
  case CHECKSUM_NONE:
    return;
  case CHECKSUM_CRC32:
...
uint32_t Checksum::getValue(ChecksumType type, uint32_t value) {
  switch (type) {
  case CHECKSUM_NONE:
    return 0;
  case CHECKSUM_CRC32:
...
const char * Checksum::validate(const char * data, uint64_t dataLen,
    const char * checksum) const {
  // TODO: implement faster 3-way instruction level parallel checksum
  uint64_t cur = 0;
  while (cur < dataLen) {
    uint64_t vlen = std::min(dataLen - cur, _bytesPerChecksum);
    if (!validateOne(data + cur, vlen, checksum)) {
      return data + cur;
    }
    cur += vlen;
    checksum += 4;
...
void Checksum::checksum(const char * data, uint64_t dataLen,
    char * checksum) const {
  // TODO: implement faster 3-way instruction level parallel checksum
  uint64_t cur = 0;
  while (cur < dataLen) {
    uint64_t vlen = std::min(dataLen - cur, _bytesPerChecksum);
    checksumOne(data + cur, vlen, checksum);
    cur += vlen;
    checksum += 4;
  }
}
...
uint32_t crc32_sb8(uint32_t value, const uint8_t *buf, size_t length) {
  ...
  for (li=0; li < running_length/8; li++) {
    value ^= *(uint32_t *)buf;
    buf += 4;
    uint32_t term1 = CRC32_T8_7[value & 0x000000FF] ^
        CRC32_T8_6[(value >> 8) & 0x000000FF];
